...
import { Search, MessageCircle, Bell, X } from 'lucide-react';
...
export default function ClientList() {
  ...
  const loadClients = async () => {
    try {
      ...
      const { data } = await api.get(`/clients/my?${params}`);
      setClients(data);
    } catch (error) {
  ...
  const sendWhatsApp = async (client: any) => {
    const phone = client.phone.replace(/\D/g, '');
    let message = `Hola ${client.name}! Soy tu asesor de Automotors.\n\nMe comunico contigo sobre el vehiculo: ${client.vehicle}\n\n`;
    ...
    if (client.appointment_date) {
      ...
      message += `Recuerda que tu cita es el ${dateStr} a las ${timeStr}.\n\n`;
    }

    message += `Tienes alguna consulta? Estoy para servirte.`;
    window.open(`https://wa.me/${phone}?text=${encodeURIComponent(message)}`, '_blank');

    try {
      await api.put(`/clients/${client.id}/status`, { status: 'en_proceso' });
      setClients(clients.map(c => c.id === client.id ? { ...c, status: 'en_proceso', last_contact_at: new Date().toISOString() } : c));
    } catch (error) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-surface-800">Mis Clientes</h2>
          <p className="text-gold-400 text-sm">{clients.length} total</p>
        </div>
        {pendingCount > 0 && (
          <div className="flex items-center gap-1.5 bg-red-500/10 text-red-400 px-3 py-1.5 rounded-full border border-red-500/20">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filteredClients.map((client) => {
            const isPending = client.status === 'sin_contacto' &&
              (!client.last_contact_at || new Date(client.last_contact_at) < new Date(Date.now() - 24 * 60 * 60 * 1000));

            return (
              <div key={client.id} className={`card-compact ${isPending ? 'border-red-500/30 bg-red-500/5' : ''}`}>
                <div className="flex items-start justify-between mb-2.5">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-surface-800 text-base truncate">{client.name}</h3>
                      {isPending && <Bell className="w-4 h-4 text-red-400 flex-shrink-0" />}
                    </div>
                ...
                <div className="flex items-center gap-2 pt-2.5 border-t border-surface-200/50">
                  <button
                ...
                <p className="text-gold-500 text-sm mt-2">{formatDate(client.created_at)}</p>
              </div>
            );
          })}
        </div>
